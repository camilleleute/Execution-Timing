...
int main(void)
{
  ...
  // Store the original value of the register to avoid setting pins unnecessarily.
  uint32_t original_odr = GPIOC->ODR;

  while (1)
  {
	  for (int i = 0; i <=15; i++) {
		  // Because the ODR bits for pins 0-4 are the lowest in the register,
		  // counting to 15 just requires ORing the count with the register.
		  GPIOC->ODR = original_odr | i;
		  // Small delay
		  for (int i2 = 2000000; i2 > 0; i2--);
	  }
  }
}
